...
(function () {
  ...
  const SUITE = [
    ...
    { g: '흐름', name: '양방향: 응답 회신 반영', expect: 'answered + 응답 일치',
      run: async () => { if (!RID) return { pass: false, got: '선행 생성 실패' }; await PARKLINK.answerRequest(RID, 'E2E 응답'); const q = await PARKLINK.getRequest(RID); return { pass: q && q.status === 'answered' && q.reply === 'E2E 응답', got: q ? (q.status + ' / ' + q.reply) : '없음' }; } },
